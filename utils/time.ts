...
export class TimeUtil {
  public static async now(provider: ethers.providers.JsonRpcProvider): Promise<number> {
    ...
    const now = TimeFormat.fromBlockTimestamp(block.timestamp);
    return now;
  }
}

export class TimeFormat {
  public static fromBlockTimestamp(timestamp: number): number {
    return Math.floor(timestamp * 1000);
  }

  public static fromBigNumber(timestamp: BigNumberish): number {
    return Math.floor(Number(timestamp.toString()) / 1000);
  }
}
